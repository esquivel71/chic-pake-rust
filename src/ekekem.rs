...
mod tests {
    ...
    //     let keypair_func = | rng: &mut rand::rngs::ThreadRng | {
    //         let alg = if cfg!(feature = "use_kyber512") {
    //             Algorithm::MlKem512
    //         } else if cfg!(feature = "use_kyber1024") {
    //             Algorithm::MlKem1024
    //         } else {
    //             Algorithm::MlKem768
    //         };
    //         let keypair = key_gen(alg, rng).unwrap();
    //         let mut pk = [0u8;KYBER_PUBLICKEYBYTES];
    //         let mut sk = [0u8;KYBER_SECRETKEYBYTES];
    //         pk.copy_from_slice(&keypair.1.encode());
    //         sk.copy_from_slice(&keypair.0.encode());
    //         (pk,sk)
    //     };
    ...
    //             Algorithm::MlKem768
    //         };
    //         let pub_key = PublicKey::decode(alg, pk.as_slice()).unwrap();
    //         let (ss_2, ct_2) = pub_key.encapsulate(rng).unwrap();
    //         let mut ct = [0u8;KYBER_CIPHERTEXTBYTES];
    //         let mut ss = [0u8;KYBER_SYMBYTES];
    //         ct[..KYBER_CIPHERTEXTBYTES].copy_from_slice(ct_2.encode().as_slice());
    //         ss[..KYBER_SYMBYTES].copy_from_slice(ss_2.encode().as_slice());
    //         (ct,ss)
    //     };
    ...
    //             Algorithm::MlKem768
    //         };
    //         let ct_2 = Ct::decode(alg, ct.as_slice()).unwrap();
    //         let priv_key = PrivateKey::decode(alg, sk.as_slice()).unwrap();
    //         let ss_2 = ct_2.decapsulate(&priv_key).unwrap();
    //         let mut ss = [0u8;KYBER_SYMBYTES];
    //         ss.copy_from_slice(&ss_2.encode());
    //         ss
    //     };
    ...
    #[test]
    #[cfg(feature = "use_kyber768")]
    fn test_chic_kyber_asm() {
        let mut sid = [0u8;KYBER_SSBYTES];
        let mut pw = [0u8;KYBER_SSBYTES];
        let mut sk = [0u8;KYBER_SECRETKEYBYTES];
        let mut pk = [0u8;KYBER_PUBLICKEYBYTES];
        ...
        let keypair_func = | rng: &mut rand::rngs::ThreadRng | {
            let (pk,sk) = kyber_asm::kyber768_keypair();
            (pk,sk)
        };
        ...
        let encapsulate_func= | pk: &[u8;KYBER_PUBLICKEYBYTES], rng: &mut rand::rngs::ThreadRng | {
            ...
            kyber_asm::kyber768_enc(&mut ct, &mut ss, pk);
            (ct,ss)
        };

        let decapsulate_func= | ct: &[u8;KYBER_CIPHERTEXTBYTES], sk: &[u8;KYBER_SECRETKEYBYTES] | {
            let mut ss = [0u8; KYBER_SSBYTES];
            kyber_asm::kyber768_dec(&mut ss, ct, sk);
            ss
        };
